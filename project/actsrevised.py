...
def get_all_categories():
    os.popen("mv /db/backnew.db /app").read()
    global api_count
    global api_count_success
    global api_count_fail
    '''
    ...
    for category in Categories:
        ...
        output[str(category.category_name)]=category.count
...
def get_all_acts_in_category(category_name):
    ...
    if Category.query.filter_by(category_name=category_name).count()==0:
        ...
        api_count+=1
        api_count_fail+=1
        os.popen("mv /app/backnew.db /db").read()
...
def upload_act():
    ...
    r=requests.get(url='http://52.87.246.4:80/api/v1/users')
    res=r.json()
...
def get_acts_in_range(category_name):
    ...
    if start_range is not None and end_range is not None:
        ...
        return jsonify([int(str(len(output)))]),200
    else:
...
def get_size_of_category(category_name):
    ...
    api_count_success+=1
    os.popen("mv /app/backnew.db /db").read()
    return jsonify([int(str(Acts.count()))]),200
...
@health_decorator
def get_api_count():
    return jsonify([int(str(api_count))])
